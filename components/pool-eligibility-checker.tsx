        ...
        {showResults && (
          <div className="space-y-2 mt-4">
            ...
            <div className="grid gap-2">
              {pools.map((pool) => {
                const eligible = isPoolEligible(pool.id, investmentAmount || "0", Number(referralsCount) || 0, pools)
                ...
                return (
                  <div key={pool.id} className="flex items-center justify-between p-2 border rounded">
                    ...
                    <div className="text-xs text-muted-foreground">
                      Min: {pool.minInvestmentAmount} 5PT / {pool.minDirectReferralsCount} refs
                    </div>
                  </div>
                )
              })}
